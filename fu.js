...
//	sys.puts(req.url + ", " + url.parse(req.url) + ", " + url.parse(req.url).pathname);	
	var pathname = url.parse(req.url).pathname;
    var handler = getMap[pathname] || notFound;
	
/*	if(!handler) {
		// var suffix = /\.\w+$/.exec(pathname);
		// suffix.toLoweCase();
		var suffix = extname(pathname).toLowerCase();
		sys.puts("suffix:" + suffix);
		if(acceptedSuffixes.indexOf(suffix) != -1) {
			sys.puts("inside");
			handler = fu.staticHandler(pathname);
		} else {
			handler = notFound;
		}
	}*/
